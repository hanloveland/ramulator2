...
class DDR5PCH : public IDRAM, public Implementation {
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      ...
      {"DDR5_3200AN",  {3200,   8,  24,  24,   24,   52,   75,   48,   12,  22,  2,    8,     8,     22+(4*8)+4,   22+(1*8)+4,  8,    32,       22+(4*8)+16,   22+(1*8)+16,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   625}},
      {"DDR5_3200BN",  {3200,   8,  26,  26,   26,   52,   77,   48,   12,  24,  2,    8,     8,     24+(4*8)+4,   24+(1*8)+4,  8,    32,       24+(4*8)+16,   24+(1*8)+16,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   625}},
      {"DDR5_3200C",   {3200,   8,  28,  28,   28,   52,   79,   48,   12,  26,  2,    8,     8,     26+(4*8)+4,   26+(1*8)+4,  8,    32,       26+(4*8)+16,   26+(1*8)+16,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   625}},
      {"DDR5_4800B",   {4800,   8,  40,  39,   39,   77,   116,  72,   18,  38,  2,    8,     8,     38+(4*8)+6,   38+(1*8)+4,  8,    49,       38+(4*8)+24,   38+(1*8)+24,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   416}},
      {"DDR5_6400AN",  {6400,   8,  46,  46,   46,   103,  149,  97,   25,  44,  2,    8,     8,     44+(4*8)+9,   44+(1*8)+9,  8,    65,       44+(4*8)+33,   44+(1*8)+33,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   312}},
      {"DDR5_7200AN",  {7200,   8,  52,  52,   52,   116,  168,  109,  28,  50,  2,    8,     8,     50+(4*8)+9,   50+(1*8)+9,  10,   73,       50+(4*8)+37,   50+(1*8)+37,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   277}},
      {"DDR5_8000AN",  {8000,   8,  56,  56,   56,   128,  184,  120,  30,  54,  4,    8,     8,     54+(4*8)+8,   54+(1*8)+8,  20,   80,       54+(4*8)+40,   54+(1*8)+40,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   250}},
      {"DDR5_8800AN",  {8800,   8,  62,  62,   62,   141,  204,  133,  34,  60,  4,    8,     8,     60+(4*8)+6,   60+(1*8)+6,  23,   89,       60+(4*8)+45,   60+(1*8)+45,   8,   -1,   -1,  -1,   -1,   -1,    -1,     30,    -1,   -1,   -1,     -1,     -1,    2,   227}},      
    };
    ...
    */
    ...
    inline static const ImplLUT m_command_scopes = LUT (
      m_commands, m_levels, {
        ...
        {"PRE_RD",  "column"},     {"PRE_RDA",   "column"},     
        {"POST_WR", "column"},     {"POST_WRA",  "column"},
        {"PRE_WR",  "rank"},       {"POST_RD", "rank"},
   ...
   ***********************************************/
    ...
    inline static constexpr ImplDef m_cmds_counted = {
      "ACT", "PRE", "RD", "WR", "REF", "RFM", "DRAM2DB_RD", "DB2DRAM_WR", "DB2MC_RD", "MC2DB_WR", "NDP_DRAM2DB_RD", "NDP_DB2DRAM_WR",
    };
  ...
  inline static const std::map<std::string, int> m_ndp_ops = {
    {"ACC_BUF"           ,0 }, 
    {"ADD"               ,1 }, 
    {"MUL"               ,2 },
    {"ADDTree"           ,3 },
  ...
  public:
    ...
    // NDP Unit per pch 
    std::vector<std::vector<uint64_t>>  ins_mem_per_pch;       // Instruction Memory of NDP Unit
    std::vector<std::vector<uint64_t>>  dat_mem_per_pch;       // Data Memory of NDP Unit
    std::vector<int>                    ndp_status_per_pch;    // Status Register of NDP Unit
    std::vector<int>                    ndp_pc_per_pch;        // Program Counter of NDP Unit
    std::vector<std::vector<Inst_Slot>> ndp_inst_slot_per_pch; // Running NDP Instruction Slot
    std::vector<int>                    loop_cnt_per_pch;      // Loop Counter of NDP Unit

    int m_num_ndp_inst_slot;
    // MC -> RCD -> DB Clock (Command Path from MC to NDP Unit)
  ...
  public:
    ...
    void count_ndp_ops(u_int32_t channel_id, u_int32_t pseudo_channel_id, uint32_t opcode, uint32_t opsize) {
      uint32_t pch_id = channel_id*num_pseudo_ch+pseudo_channel_id;
      if(opcode == m_ndp_inst_op.at("LOAD")) {
        // WRITE
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += opsize;
      } else if(opcode == m_ndp_inst_op.at("LOAD_ADD")) {
        // ADD - WRITE
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += opsize;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]     += opsize;
      } else if(opcode == m_ndp_inst_op.at("LOAD_MUL")) {
        // MUL - WRITE
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += opsize;      
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("MUL")]     += opsize;
      } else if(opcode == m_ndp_inst_op.at("ADD")) {
        // READ - ADD- WRITE
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += opsize * 2;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]     += opsize;
      } else if(opcode == m_ndp_inst_op.at("MUL")) {
        // READ - MUL - WRITE
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += opsize * 2;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("MUL")]     += opsize;        
      } else if(opcode == m_ndp_inst_op.at("V_RED")) {
        // READ (one) - ADD - WRITE (one)
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += (2);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]     += opsize + 1;
      } else if(opcode == m_ndp_inst_op.at("S_RED")) {
        // ADDTree - WRITE 
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")] += (opsize);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADDTree")] += opsize;
      } else if(opcode == m_ndp_inst_op.at("MAC")) {
        // READ - ADD - MUL - WRITE (One)
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize + 1);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]    += opsize + 1;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("MUL")]    += opsize;
      } else if(opcode == m_ndp_inst_op.at("SCALE_ADD")) {
        // READ - MUL - ADD - WRITE 
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize*2);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]    += opsize;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("MUL")]    += opsize;
      } else if(opcode == m_ndp_inst_op.at("T_ADD")) {
        // READ x 2 - ADD - WRITE 
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize*3);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]    += opsize;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("IMPL_EXEC")] += opsize;        
      } else if(opcode == m_ndp_inst_op.at("T_V_RED")) {
        // READ(one) - READ - ADD - WRITE (one)
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize + 2);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]    += opsize + 1;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("IMPL_EXEC")] += opsize;
      } else if(opcode == m_ndp_inst_op.at("T_S_RED")) {
        // READ - ADDTree - WRITE 
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize * 2);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADDTree")]+= opsize;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("IMPL_EXEC")] += opsize;
      } else if(opcode == m_ndp_inst_op.at("T_MAC")) {
        // READ - READ - MUL - ADD - WRITE 
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ACC_BUF")]+= (opsize*3);
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("ADD")]    += opsize + 1;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("MUL")]    += opsize;
        ndp_op_cnt_per_pch[pch_id][m_ndp_ops.at("IMPL_EXEC")] += opsize;
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      io_boost = param_group("org").param<int>("io_boost").default_val(1);
      if(!(io_boost == 1 || io_boost == 2 || io_boost == 4)) {
        throw ConfigurationError("IO Boost can be configured to 1, 2 or 4, but not {}", io_boost);
      }
      m_read_latency = m_timing_vals("nCL") + m_timing_vals("nBL") * (4 / io_boost);

      constexpr int nCCDS_WTR_TABLE[3][6] = {
      // 3200,4800,6400,7200,8000,8800  
        {(22+(4*8)+4),(38+(4*8)+6),(44+(4*8)+9),(50+(4*8)+9),(54+(4*8)+8),(60+(4*8)+6)}, // IO BOOST 1
        {(22+(2*8)+4),(38+(2*8)+6),(44+(2*8)+9),(50+(2*8)+9),(54+(2*8)+8),(60+(1*8)+6)}, // IO BOOST 2
        {(22+(1*8)+4),(38+(1*8)+6),(44+(1*8)+9),(50+(1*8)+9),(54+(1*8)+8),(60+(1*8)+6)}, // IO BOOST 4
      };
      constexpr int nCCDL_WTR_TABLE[3][6] = {
      // 3200,4800,6400,7200,8000,8800  
        {(22+(4*8)+16),(38+(4*8)+24),(44+(4*8)+33),(50+(4*8)+37),(54+(4*8)+40),(60+(4*8)+45)}, 
        {(22+(2*8)+16),(38+(2*8)+24),(44+(2*8)+33),(50+(2*8)+37),(54+(2*8)+40),(60+(2*8)+45)}, 
      ...
      // When Use x8 or x16 DRAM devcie
      if(dq_scaling == 2 || dq_scaling == 4) {
        int old_nCL = m_timing_vals("nCL");
        int old_nWR = m_timing_vals("nWR");
        m_timing_vals("nCL") = m_timing_vals("nCL") + (int)(10*1000/m_timing_vals("tCK_ps"));
        m_timing_vals("nWR") = m_timing_vals("nWR") + (int)(10*1000/m_timing_vals("tCK_ps"));
        m_logger->info(" USE x8 or x16 DRAM Device!");      
        m_logger->info("  - Timing Parameter Change nCL form {} to {}",old_nCL,m_timing_vals("nCL"));      
        m_logger->info("  - Timing Parameter Change nWL form {} to {}",old_nWR,m_timing_vals("nWR"));      
      }
      ...
      populate_timingcons(this, {
          ...
          {.level = "narrowio", .preceding = {"RD", "RDA", "POST_RD", "NDP_DB_RD"}, .following = {"RD", "RDA", "POST_RD", "NDP_DB_RD"},  .latency = (V("nBL")*(4/io_boost))},
          {.level = "narrowio", .preceding = {"RD", "RDA", "POST_RD", "NDP_DB_RD"}, .following = {"WR", "WRA", "PRE_WR", "NDP_DB_WR"},   .latency = V("nCL") + (V("nBL")*4/io_boost) + 2 - V("nCWL") + 2},
          {.level = "narrowio", .preceding = {"WR", "WRA", "PRE_WR", "NDP_DB_WR"},  .following = {"WR", "WRA", "PRE_WR", "NDP_DB_WR"},   .latency = (V("nBL")*4/io_boost)},          
          ...
          {.level = "wideio", .preceding = {"WR", "WRA"},                                                       .following = {"WR", "WRA", "POST_WR", "POST_WRA", "NDP_DRAM_WR", "NDP_DRAM_WRA"}, .latency = (V("nBL")*4/io_boost)},
          ...
          {.level = "rank", .preceding = {"RD",     "RDA"},                                      .following = {"WR", "WRA", "POST_WR", "POST_WRA", "NDP_DRAM_WR", "NDP_DRAM_WRA"}, .latency = V("nCL") + (V("nBL")*4/io_boost) + 2 - V("nCWL") + 2},   // nCCDS_RTW
          ...
          {.level = "rank", .preceding = {"RD", "RDA"}, 
                            ...
                            .latency = (V("nBL")*4/io_boost) + V("nCS"), .is_sibling = true},
          ...
          {.level = "rank", .preceding = {"WR", "WRA"}, 
                            ...
                            .latency = V("nCL")  + (V("nBL")*4/io_boost) + V("nCS") - V("nCWL"), .is_sibling = true},
          ...
          {.level = "rank", .preceding = {"WR"},                          .following = {"PREA"}, .latency = V("nCWL") + (V("nBL")*4/io_boost) + V("nWR")},          
          ...
          {.level = "rank", .preceding = {"WRA"}, .following = {"REFab"}, .latency = V("nCWL") + (V("nBL")*4/io_boost) + V("nWR") + V("nRP")},   
          ...
          {.level = "bank", .preceding = {"WR"},                           .following = {"PRE", "PREsb"}, .latency = V("nCWL") + (V("nBL")*4/io_boost) + V("nWR")},  
          ...
          {.level = "bank", .preceding = {"WRA"},                       .following = {"ACT", "REFsb"}, .latency = V("nCWL") + (V("nBL")*4/io_boost) + V("nWR") + V("nRP")},  
          {.level = "bank", .preceding = {"POST_WRA", "NDP_DRAM_WRA"},  .following = {"ACT", "REFsb"}, .latency = V("nCWL") + V("nBL") + V("nWR") + V("nRP")},  
          {.level = "bank", .preceding = {"WR"},                        .following = {"RDA"}, .latency = V("nCWL") + (V("nBL")*4/io_boost) + V("nWR") - V("nRTP")},  
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["NDP_DRAM_RD"]]  = Lambdas::Power::Bank::NDP_DRAM2DB_RD<DDR5PCH>;
      m_powers[m_levels["bank"]][m_commands["NDP_DRAM_RDA"]] = Lambdas::Power::Bank::NDP_DRAM2DB_RD<DDR5PCH>;
      m_powers[m_levels["bank"]][m_commands["NDP_DRAM_WR"]]  = Lambdas::Power::Bank::NDP_DB2DRAM_WR<DDR5PCH>;
      m_powers[m_levels["bank"]][m_commands["NDP_DRAM_WRA"]] = Lambdas::Power::Bank::NDP_DB2DRAM_WR<DDR5PCH>;
    ...
    void finalize() override {
      ...
      //  -- Each NDP_DRAM_WR
      // Decoder: 12.50 pJ per RS(10,8,8) x8 (Per Access)
      //  -- Each NDP_DRAM_RD
      double ecc_decoder_energy = 12.5;
      double ecc_encoder_energy = 1.13;

      // energy per ops (pJ) (ref: Mark Horowitz.1.1 computing’s energy problem (and what we can do about it).
      // 45nm technology
      double fp16_add_energy = 0.4;
      double fp16_mul_energy = 1.1;
      ...
      // Energy Per 256-bits 
      //               | x4    | x8    | x16
      // E_NDP_IDLE	   | 77.8  | 155.6 | 311.2 
      // E_NDP_ACC_MEM | 105.4 | 183.2 | 338.8 
      // E_NDP_EXEC    | 301.5 | 602.9 | 1205.8 
      double ndp_idle_per_ndp_cycle;
      double ndp_acc_buf_per_inst;
      double ndp_exec_per_inst;

      if(dq_scaling == 1) {
        ndp_idle_per_ndp_cycle = 77.8; 
        ndp_acc_buf_per_inst = 105.4 * 2;
        ndp_exec_per_inst = 301.5 * 2;
      } else if(dq_scaling == 2) {
        ndp_idle_per_ndp_cycle = 155.6; 
        ndp_acc_buf_per_inst  = 183.2 * 2;
        ndp_exec_per_inst     = 602.9 * 2;
      } else if (dq_scaling == 4) {
        ndp_idle_per_ndp_cycle = 311.2; 
        ndp_acc_buf_per_inst  = 338.8 * 2;
        ndp_exec_per_inst     = 1205.8 * 2;
      }
      ...
      size_t host_acc = 0; // Host <->DB 
      size_t db_acc   = 0; // DB <-> DRAMs
      size_t ndp_exec  = 0;
      for (int i = 0; i < num_channels; i++) {
        size_t num_socket_trans = 0;
        size_t num_on_board_trans = 0;
        size_t num_access_buf_trans = 0;
        size_t num_ecc_decoder_trans = 0;
        size_t num_ecc_encoder_trans = 0;
        size_t num_ndp_acc_mem = 0;
        size_t num_ndp_add = 0;
        size_t num_ndp_mul = 0;
        size_t num_ndp_addtree = 0;
        size_t num_ndp_impl_exec = 0;
        size_t ndp_explit_exec   = 0;
        uint64_t idle_ndp_cycle = 0;
        for (int k = 0; k < num_pseudochannels; k++) {
            for (int j = 0; j < num_ranks; j++) {
            ...
            num_on_board_trans+=   m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("RD")] + 
                                   ...
                                   m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("NDP_DRAM2DB_RD")] +  
                                   m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("NDP_DB2DRAM_WR")];                                   
            ...
            num_ecc_encoder_trans += m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("DB2DRAM_WR")] + 
                                     m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("NDP_DB2DRAM_WR")] +
                                     m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("DB2MC_RD")];        
            ndp_explit_exec += m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("NDP_DRAM2DB_RD")] +   
                               m_power_stats[i * num_ranks * num_pseudochannels + k * num_ranks + j].cmd_counters[m_cmds_counted("NDP_DB2DRAM_WR")];         
          }
          ...
          num_ndp_impl_exec += ndp_op_cnt_per_pch[i * num_pseudochannels + k][m_ndp_ops.at("IMPL_EXEC")];
          
          // NDP Clock m_clk/4 (NDP_IDLE_POWER)
          idle_ndp_cycle += (m_clk/4);
        }
        host_acc += num_socket_trans;
        db_acc   += num_on_board_trans;
        ndp_exec += ndp_explit_exec;
        idle_ndp_cycle = idle_ndp_cycle - (ndp_explit_exec * 2) - num_ndp_impl_exec;
        ...
        double channel_access_buf_energy = (double) (num_access_buf_trans) * ndp_acc_buf_per_inst / 1E3;
        ...
        double channel_ndp_energy = ((double)(ndp_explit_exec + num_ndp_impl_exec/2)  * ndp_exec_per_inst + 
                                     (double)(idle_ndp_cycle) * ndp_idle_per_ndp_cycle) / 1E3;                                          
        ...
        std::cout<<" - DQ (ECC) Energy (nJ)     : "<<channel_onboard_ecc_energy<<std::endl;
        std::cout<<" - NDP Ops Energy (nJ)      : "<<channel_ndp_energy<<std::endl;
        ...
        std::cout<<" - DQ (ECC) Power (W)       : "<<channel_onboard_ecc_power<<std::endl; 
        std::cout<<" - NDP Ops Power (W)        : "<<channel_ndp_power<<std::endl; 
      }
      ...
      std::cout<<" - NDP Ops Energy (nJ)          : "<<total_ndp_energy<<std::endl;
      ...
      total_ndp_bw  = (double)((double)ndp_exec * 512.0 * (double)dq_scaling) / ((double)m_clk * (double)m_timing_vals("tCK_ps") / 1000.0) / 8;            
      ...
      std::cout<<" - Total NDP Access                 : "<<ndp_exec<<std::endl;
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      double rd_cmd_energy   = (VE("VDD") * (CE("IDD4R") - CE("IDD3N")) + VE("VPP") * (CE("IPP4R") - CE("IPP3N"))) 
                                      * (rank_stats.cmd_counters[m_cmds_counted("RD")] + rank_stats.cmd_counters[m_cmds_counted("DRAM2DB_RD")] + rank_stats.cmd_counters[m_cmds_counted("NDP_DRAM2DB_RD")]) * TS("nBL") * tCK_ns / 1E3;
                                      
      double wr_cmd_energy   = (VE("VDD") * (CE("IDD4W") - CE("IDD3N")) + VE("VPP") * (CE("IPP4W") - CE("IPP3N"))) 
                                      * (rank_stats.cmd_counters[m_cmds_counted("WR")] + rank_stats.cmd_counters[m_cmds_counted("DB2DRAM_WR")] + rank_stats.cmd_counters[m_cmds_counted("NDP_DB2DRAM_WR")]) * TS("nBL") * tCK_ns / 1E3;
      ...
      double rfm_cmd_energy = (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) * num_bankgroups
        ...
        std::cout<<"rank_stats.cmd_counters[DRAM2DB_RD]   : "<<rank_stats.cmd_counters[m_cmds_counted("DRAM2DB_RD")]<<std::endl;
        std::cout<<"rank_stats.cmd_counters[DB2DRAM_WR]   : "<<rank_stats.cmd_counters[m_cmds_counted("DB2DRAM_WR")]<<std::endl;
      #endif
      ...
      std::cout<<" - Background Power (W)   : "<<background_power<< " / "<< s_total_background_power <<std::endl;
