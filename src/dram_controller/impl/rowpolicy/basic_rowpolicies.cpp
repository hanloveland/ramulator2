...
class OpenRowPolicy : public IRowPolicy, public Implementation {
  ...
  public:
    ...
    void update_cap(int pch, int rank, int bg, int bk, uint64_t cap) override {
      // DO NOT ANYTHING
    };
...
class ClosedRowPolicy : public IRowPolicy, public Implementation {
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_cap_per_bank.resize(m_num_banks * m_num_bankgroups * m_num_ranks, m_cap);
    ...
    void update_cap(int pch, int rank, int bg, int bk, uint64_t cap) override {
      int flat_bank_id = bk + bg * m_num_banks + rank * m_num_banks * m_num_bankgroups;
      m_cap_per_bank[flat_bank_id] = cap;
    };    
};

class ClosedRowPolicyPch : public IRowPolicy, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IRowPolicy, ClosedRowPolicyPch, "ClosedRowPolicyPch", "Close Row Policy for DDR5 with Pseudo-Channel.")
  private:
    IDRAM* m_dram;
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_pseudochannel_level = m_dram->m_levels("pseudochannel");
      m_rank_level = m_dram->m_levels("rank");
      m_bankgroup_level = m_dram->m_levels("bankgroup");
      m_bank_level = m_dram->m_levels("bank");
      m_row_level = m_dram->m_levels("row");
      ...
      {  
        ...
        if (req_it->addr_vec[m_bankgroup_level] == -1 && req_it->addr_vec[m_bank_level] == -1) {  // all bank closes
          for (int b = 0; b < m_num_banks; b++) {
            for (int bg = 0; bg < m_num_bankgroups; bg++) {
              int rank_id = req_it->addr_vec[m_rank_level];
              int pch_id =  req_it->addr_vec[m_pseudochannel_level];
              int flat_bank_id = b + bg * m_num_banks + rank_id * m_num_banks * m_num_bankgroups + pch_id * m_num_banks * m_num_bankgroups * m_num_ranks;
              m_col_accesses[flat_bank_id] = 0;
            }
          }
        } else if (req_it->addr_vec[m_bankgroup_level] == -1) {  // same bank closes
          for (int bg = 0; bg < m_num_bankgroups; bg++) {
            ...
            int rank_id = req_it->addr_vec[m_rank_level];
            int pch_id =  req_it->addr_vec[m_pseudochannel_level];
            int flat_bank_id = bank_id + bg * m_num_banks + rank_id * m_num_banks * m_num_bankgroups + pch_id * m_num_banks * m_num_bankgroups * m_num_ranks;
            m_col_accesses[flat_bank_id] = 0;
          }
        } else {  // single bank closes  (PRE, VRR, RDA, WRA)
          int flat_bank_id = req_it->addr_vec[m_bank_level] + 
                             req_it->addr_vec[m_bankgroup_level] * m_num_banks + 
                             req_it->addr_vec[m_rank_level] * m_num_banks * m_num_bankgroups +
                             req_it->addr_vec[m_pseudochannel_level] * m_num_banks * m_num_bankgroups * m_num_ranks;
