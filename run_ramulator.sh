...
ndp_4x_trace_list=(
"ndp_4x_8K_AXPBY.txt"
"ndp_4x_32K_AXPBY.txt" 
"ndp_4x_128K_AXPBY.txt"
"ndp_4x_512K_AXPBY.txt"  
"ndp_4x_2M_AXPBY.txt"
"ndp_4x_8M_AXPBY.txt"
"ndp_4x_8K_AXPBYPCZ.txt"
"ndp_4x_32K_AXPBYPCZ.txt" 
"ndp_4x_128K_AXPBYPCZ.txt"
"ndp_4x_512K_AXPBYPCZ.txt"  
"ndp_4x_2M_AXPBYPCZ.txt"
"ndp_4x_8M_AXPBYPCZ.txt"
"ndp_4x_8K_AXPY.txt"
"ndp_4x_32K_AXPY.txt" 
"ndp_4x_128K_AXPY.txt"
"ndp_4x_512K_AXPY.txt"  
"ndp_4x_2M_AXPY.txt"
"ndp_4x_8M_AXPY.txt"
"ndp_4x_8K_COPY.txt"
"ndp_4x_32K_COPY.txt" 
"ndp_4x_128K_COPY.txt"
"ndp_4x_512K_COPY.txt"  
"ndp_4x_2M_COPY.txt"
"ndp_4x_8M_COPY.txt"
"ndp_4x_8K_XMY.txt"
"ndp_4x_32K_XMY.txt" 
...
for speed_grade in ${speed_grade_list[@]}; do
    echo " -- Speed Grade: "$speed_grade
        for dbx_org in ${dbx_org_list[@]}; do
            ...
            real_dq="4"
            current_preset="DDR5_4800x4"
            mux_ndp_trace_list=("${ndp_4x_trace_list[@]}")
            if [[ $dbx_org == "DDR5_16Gb_DBX_x8" ]]; then 
                real_dq="8"
                current_preset="DDR5_4800x8"
                mux_ndp_trace_list=("${ndp_8x_trace_list[@]}")
            elif [[ $dbx_org == "DDR5_16Gb_DBX_x16" ]]; then 
                real_dq="16"
                current_preset="DDR5_4800x16"
                mux_ndp_trace_list=("${ndp_16x_trace_list[@]}")
            fi
            speed_option="MemorySystem.DRAM.timing.preset="$speed_grade
            real_dq_option="MemorySystem.DRAM.org.real_dq="$real_dq
            current_option="MemorySystem.DRAM.current.preset="$current_preset
            ...
            echo " -- Real DQ on DIMM : "$real_dq_option
            echo " -- Current: "$current_option
            for trace in ${mux_ndp_trace_list[@]}; do
                trace_path="Frontend.path=./trace/ndp/${trace}"
                log_path="${LOG_PATH}/ndp/$speed_grade_${trace}.log"
                echo "========== Run Ramulator ============"
                echo "Trace: "$trace
                cmd="./ramulator2 -f $conf_file -p  $trace_path -p $speed_option -p $dbx_org_option -p $real_dq_option -p $current_option > $log_path &"
                echo $cmd
                eval $cmd
                ((num_process++))
                if [[ $num_process == 8 ]]; then 
                    num_process=0
                    wait
                fi
             done
