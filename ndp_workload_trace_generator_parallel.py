...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Parallel NDP trace generator")
    parser.add_argument("-j", "--workers", type=int, default=os.cpu_count() or 8,
                        help="Number of parallel workers (default: CPU count)")
    parser.add_argument("-o", "--output", type=str, default="generated_traces_p",
                        help="Output root directory (default: generated_traces_p)")
    args = parser.parse_args()

    BLAS_WORKLOADS = ["COPY", "AXPY", "AXPBY", "AXPBYPCZ", "XMY", "DOT"]
    BLAS_SIZES = ["8K", "128K", "2M"]
    GEMV_SIZES = ["8K", "32K", "128K"]
    MAX_WORKERS = args.workers
    root_path = args.output

    GEN_EXP1_BASE_STANDALONE   = True
    GEN_EXP2_SCALING_DRAM      = True
    GEN_EXP3_SCALING_DIMM      = True
    ...
    if GEN_SLS_BASELINE:
        ...
        setup_dir(sls_root)
        print("\n[SLS] Baseline")
        run_parallel([(cfg, sls_root) for cfg in sls_config_names], _worker_sls_trace, "SLS")
        print("[SLS] Done.")

    if GEN_SLS_DBX:
        sls_dbx_root = root_path + "/sls_dbx"
        setup_dir(sls_dbx_root)
        print("\n[SLS-DBX] DBX-DIMM NDP")
        run_parallel([(cfg, sls_dbx_root) for cfg in sls_config_names], _worker_sls_pch_trace, "SLS-DBX")
        print("[SLS-DBX] Done.")
    ...
    if GEN_EXP2_SCALING_DRAM:
        ...
        for d in [exp2_baseline, exp2_pch_non_ndp, exp2_pch_ndp]:
            os.makedirs(d, exist_ok=True)
        print("\n[Exp2] Scaling DRAM Device: 1 DIMM, x4/x8/x16")
        trace_tasks = []
        for bench in BLAS_WORKLOADS:
            for size in BLAS_SIZES:
